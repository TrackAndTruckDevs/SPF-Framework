...
inline std::string ReplaceTemplates(std::string signature) {
  std::vector<std::pair<std::string, std::string>> sorted(kPatternTemplates.begin(), kPatternTemplates.end());
  std::sort(sorted.begin(), sorted.end(), [](const auto& a, const auto& b) { return a.first.length() > b.first.length(); });
