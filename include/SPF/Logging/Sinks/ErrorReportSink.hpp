...
struct LogReportEntry {
    ...
    bool operator==(const LogReportEntry& other) const {
        return loggerName == other.loggerName && level == other.level && message == other.message;
    }
};
...
struct LogReportEntryHash {
    std::size_t operator()(const LogReportEntry& e) const {
        return std::hash<std::string>{}(e.loggerName) ^ 
               (std::hash<int>{}(static_cast<int>(e.level)) << 1) ^ 
               (std::hash<std::string>{}(e.message) << 2);
    }
};
