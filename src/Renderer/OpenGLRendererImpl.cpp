...
class OpenGLTexture : public ITexture {
 public:
  OpenGLTexture(GLuint id, uint32_t width, uint32_t height) : m_id(id), m_width(width), m_height(height) {}

  ~OpenGLTexture() override { glDeleteTextures(1, &m_id); }
...
OpenGLRendererImpl::OpenGLRendererImpl(Renderer& renderer, UI::UIManager& uiManager) : RendererBase(renderer), m_uiManager(uiManager), m_onInitSink(OpenGLHook::OnInit), m_onPresentSink(OpenGLHook::OnPresent) {
  m_logger = LoggerFactory::GetInstance().GetLogger("OpenGLImpl");
  m_logger->Info("OpenGL Renderer Implementation created.");
}

OpenGLRendererImpl::~OpenGLRendererImpl() { Shutdown(); }
