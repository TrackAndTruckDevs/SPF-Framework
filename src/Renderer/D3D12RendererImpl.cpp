...
class D3D12Texture : public ITexture {
 public:
  D3D12Texture(ComPtr<ID3D12Resource> resource, D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle, uint32_t width, uint32_t height) : m_resource(resource), m_gpuHandle(gpuHandle), m_width(width), m_height(height) {}
...
std::unique_ptr<ITexture> D3D12RendererImpl::CreateTextureFromMemory(const unsigned char* data, size_t size) {
  ...
  HRESULT hr = m_pd3dDevice->CreateCommittedResource(&defaultHeapProps, D3D12_HEAP_FLAG_NONE, &txtDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(texture.GetAddressOf()));
  ...
  hr = m_pd3dDevice->CreateCommittedResource(&uploadHeapProps, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(uploadBuffer.GetAddressOf()));
...
void D3D12RendererImpl::OnD3D12Present(IDXGISwapChain3* swapChain) {
  ...
  if (FAILED(hr)) {
    m_logger->Error("OnD3D12Present: CommandAllocator->Reset failed. (HRESULT: {:#x})", static_cast<unsigned int>(hr));
    return;
  }

  hr = m_commandList->Reset(m_commandAllocator.Get(), NULL);
  if (FAILED(hr)) {
    m_logger->Error("OnD3D12Present: CommandList->Reset failed. (HRESULT: {:#x})", static_cast<unsigned int>(hr));
    return;
  }
  ...
  hr = m_commandList->Close();
  if (FAILED(hr)) {
    m_logger->Error("OnD3D12Present: CommandList->Close failed. (HRESULT: {:#x})", static_cast<unsigned int>(hr));
    return;
  }
...
void D3D12RendererImpl::CreateRenderTarget(IDXGISwapChain3* swapChain) {
  ...
  hr = swapChain->GetDesc(&desc);
  if (FAILED(hr)) {
    m_logger->Error("CreateRenderTarget: swapChain->GetDesc failed. (HRESULT: {:#x})", static_cast<unsigned int>(hr));
    return;
  }
  ...
  if (FAILED(hr)) {
    m_logger->Error("CreateRenderTarget: CreateDescriptorHeap (RTV) failed. (HRESULT: {:#x})", static_cast<unsigned int>(hr));
    return;
  }
  ...
  for (UINT i = 0; i < desc.BufferCount; i++) {
    ...
    if (FAILED(hr)) {
      m_logger->Error("CreateRenderTarget: swapChain->GetBuffer failed for buffer {}. (HRESULT: {:#x})", i, static_cast<unsigned int>(hr));
      return;
    }
