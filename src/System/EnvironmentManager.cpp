...
void EnvironmentManager::SetGameData(const std::string& fullName, Telemetry::Game gameId) {
  if (gameId == Telemetry::Game::ETS2) {
    m_game.code = "eut2";
    m_game.steamAppId = 227300;
  } else if (gameId == Telemetry::Game::ATS) {
    m_game.code = "ats";
...
void EnvironmentManager::RefreshDynamicStatus() {
    ...
    if (gamePtr != 0) {
      ...
      if (profileHandle != 0) {
        ...
        if (profileObj != 0) {
          ...
          switch (type) {
            case 4:
              m_status.profileType = "Steam Cloud";
              break;
            case 0:
              m_status.profileType = "Preview";
              break;
            case 1:
              m_status.profileType = "Academy";
              break;
            case 2:
              m_status.profileType = "Demo";
              break;
            default:
              m_status.profileType = "Local";
              break;
          }
        }
      }
  ...
  m_status.isVR = (cmd.find(L"-oculus") != std::wstring::npos || cmd.find(L"-openvr") != std::wstring::npos || GetModuleHandleA("openvr_api.dll") != NULL);
  ...
  if (renderer) {
    switch (renderer->GetDetectedAPI()) {
      case Rendering::RenderAPI::D3D11:
        m_status.renderer = "DirectX 11";
        break;
      case Rendering::RenderAPI::D3D12:
        m_status.renderer = "DirectX 12";
        break;
      case Rendering::RenderAPI::OpenGL:
        m_status.renderer = "OpenGL";
        break;
      default:
        m_status.renderer = "Unknown";
        break;
    }
  }
