  ...
  for (int axisIdx = 0; axisIdx < 8; ++axisIdx) {
    ...
    if (isGamepad && deviceType == SPF::System::DeviceType::Xbox) {
      if (axisIdx == 0)
        ...
        mappedIdx = 6;  // Slider 0
      else if (axisIdx == 7)
        mappedIdx = 7;  // Slider 1
      else
        mappedIdx = -1;
...
static void ProcessPovData(IDirectInputDevice8W* self, PerDeviceState& state, DIDEVICEOBJECTDATA& data, SPF::System::DeviceType type) {
  int povIdx = -1;
  if (data.dwOfs == DIJOFS_POV(0))
    povIdx = 0;
  else if (data.dwOfs == DIJOFS_POV(1))
    povIdx = 1;
  else if (data.dwOfs == DIJOFS_POV(2))
    povIdx = 2;
  else if (data.dwOfs == DIJOFS_POV(3))
    povIdx = 3;
  ...
  for (int j = 0; j < 4; ++j) {
    ...
    if (curActive != prevActive)
      blocked = inputManager.PublishGamepadEvent(SPF::Input::GamepadEvent{0, dirs[j], curActive, curActive ? 1.0f : 0.0f}, 2);
    else if (curActive)
      blocked = inputManager.ProcessAndDecide(SPF::Input::GamepadEvent{0, dirs[j], true, 1.0f}, 2);
  ...
  int reconstructed = -1;
  if (activeToGame[0] && activeToGame[1])
    reconstructed = 4500;
  else if (activeToGame[2] && activeToGame[1])
    reconstructed = 13500;
  else if (activeToGame[2] && activeToGame[3])
    reconstructed = 22500;
  else if (activeToGame[0] && activeToGame[3])
    reconstructed = 31500;
  else if (activeToGame[0])
    reconstructed = 0;
  else if (activeToGame[1])
    reconstructed = 9000;
  else if (activeToGame[2])
    reconstructed = 18000;
  else if (activeToGame[3])
    reconstructed = 27000;
...
static void HandleJoystickData(IDirectInputDevice8W* self, const DIDEVICEINSTANCEW& instance, DIDEVICEOBJECTDATA* rgdod, DWORD* pdwInOut, DWORD capacity, DWORD& sequence) {
  ...
  for (DWORD i = 0; i < *pdwInOut; ++i) {
    ...
    if (rgdod[i].dwOfs >= DIJOFS_BUTTON0 && rgdod[i].dwOfs < DIJOFS_BUTTON0 + 128) {
      ...
      if (pressed) {
        if (gamepadBtn != SPF::System::GamepadButton::Unknown)
          block = inputManager.IsGamepadButtonBlocked(gamepadBtn);
        else
          block = inputManager.IsJoystickButtonBlocked(btnIdx);
      }
      ...
      if (pressed != state.buttonStates[btnIdx]) {
        ...
        if (gamepadBtn != SPF::System::GamepadButton::Unknown)
          managerBlock = inputManager.PublishGamepadEvent(SPF::Input::GamepadEvent{0, gamepadBtn, pressed, pressed ? 1.0f : 0.0f}, 2);
        else
          managerBlock = inputManager.PublishJoystickEvent(SPF::Input::JoystickEvent{btnIdx, pressed}, 2);
    ...
    // 3. Axes
      ...
      if (rgdod[i].dwOfs == DIJOFS_X)
        ...
        axisIdx = 5;
      else if (rgdod[i].dwOfs == DIJOFS_SLIDER(0))
        axisIdx = 6;
      else if (rgdod[i].dwOfs == DIJOFS_SLIDER(1))
        axisIdx = 7;
      ...
      if (inputManager.ConsumeGamepadReleaseRequest(btn)) {
        for (DWORD off = DIJOFS_BUTTON0; off < DIJOFS_BUTTON0 + 32; ++off) {
          if (mapping.FromDInput(off) == btn) {
            InjectVirtualEvent(rgdod, writeIdx, capacity, off, 0, sequence);
            break;
          }
        }
      }
