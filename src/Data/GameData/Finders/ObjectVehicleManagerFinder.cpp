...
bool ObjectManagerFinder::TryFindOffsets(GameObjectVehicleService& owner) {
  ...
  if (scanPos) {
    ...
    if (addrMov) {
      ...
      if (addrCall) {
        ...
        logger->Warn("[Block 1] FAILED to find ClearLocalVehicles call near 0x{:X}", addrMov);
      }
    } else {
      logger->Warn("[Block 1] FAILED to find TrafficManager pointer load near 0x{:X}", scanPos);
    }
  } else {
    logger->Warn("[Block 1] FAILED to find Macro string anchor '{}' or its XREF", MACRO_CREATED_VEHICLES_STR);
  }
  // --- END OF BLOCK 1 ---
  ...
  if (pfnClear) {
      ...
      if (PatternFinder::IsSaneOffset(off)) {
        ...
        if (addrCount) {
          ...
          if (PatternFinder::IsSaneOffset(offCount)) {
            ...
            logger->Error("[Block 1.1] Vehicle Count offset INVALID (0x{:X})", offCount);
          }
        } else {
          logger->Error("[Block 1.1] FAILED to find Vehicle Count anchor");
        }
      } else {
        logger->Error("[Block 1.1] Vehicle Array offset INVALID (0x{:X})", off);
      }
    } else {
      logger->Error("[Block 1.1] FAILED to find Vehicle Array anchor");
    }
    ...
    if (addrSize) {
      ...
      logger->Error("[Block 1.1] FAILED to find Vehicle Struct Size anchor");
    }
  }
  ...
  if (idStrXref) {
    ...
    if (addrIdLoad) {
      ...
      if (PatternFinder::IsSaneOffset(offset)) {
        ...
        logger->Error("[Block 2] Vehicle ID offset INVALID (0x{:X})", offset);
      }
    } else {
      logger->Error("[Block 2] FAILED to find Vehicle ID load anchor near 0x{:X}", idStrXref);
    }
  } else {
    logger->Warn("[Block 2] FAILED to find ID string anchor '{}'", VEHICLE_ID_STR_ANCHOR);
  }
  // --- END OF BLOCK 2 ---
  ...
  if (addrPlayerLoad) {
    ...
    if (PatternFinder::IsSaneOffset(offset)) {
      ...
      logger->Error("[Block 3] LocalPlayerController offset INVALID (0x{:X})", offset);
    }
  } else {
    logger->Warn("[Block 3] FAILED to find LocalPlayerController sequence");
  }
  // --- END OF BLOCK 3 ---
  ...
  if (pfnUpdateAllTraffic) {
    ...
    if (addrActorLoad) {
      ...
      if (PatternFinder::IsSaneOffset(offset)) {
        ...
        logger->Error("[Block 3.1] PlayerVehicle offset INVALID (0x{:X})", offset);
      }
    } else {
      logger->Error("[Block 3.1] FAILED to find PlayerVehicle actor sequence inside function");
    }
  } else {
    logger->Warn("[Block 3.1] FAILED to find string anchor '{}'", SPAWNED_VEHICLE_ARRAY_STR);
  }
  // --- END OF BLOCK 3.1 ---
