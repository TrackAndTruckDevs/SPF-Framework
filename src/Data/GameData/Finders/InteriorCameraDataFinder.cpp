...
bool InteriorCameraDataFinder::TryFindOffsets(GameDataCameraService& owner) {
  ...
  logger->Info("Searching for Interior Camera offsets...");
  ...
  // --- Find offsets within UpdateInteriorCamera ---
  {
    ...
    for (int i = 0; i < 2048; ++i) {
      ...
      if (!f[0] && memcmp((void*)addr, p_head_x, sizeof(p_head_x)) == 0) {
        owner.SetInteriorSeatXOffset(*(int32_t*)(addr + 4));
        owner.SetInteriorSeatYOffset(*(int32_t*)(addr + 4) + 4);
        f[0] = true;
      }
      if (!f[1] && memcmp((void*)addr, p_head_z, sizeof(p_head_z)) == 0) {
        owner.SetInteriorSeatZOffset(*(int32_t*)(addr + 2));
        f[1] = true;
      }
      if (!f[2] && memcmp((void*)addr, p_limit_l, sizeof(p_limit_l)) == 0) {
        owner.SetInteriorLimitLeftOffset(*(int32_t*)(addr + 4));
        f[2] = true;
      }
      if (!f[3] && memcmp((void*)addr, p_limit_r, sizeof(p_limit_r)) == 0) {
        owner.SetInteriorLimitRightOffset(*(int32_t*)(addr + 4));
        f[3] = true;
      }
      if (!f[4] && memcmp((void*)addr, p_default_lr, sizeof(p_default_lr)) == 0) {
        owner.SetInteriorMouseLRDefaultOffset(*(int32_t*)(addr + 4));
        f[4] = true;
      }
      if (!f[5] && memcmp((void*)addr, p_limit_u, sizeof(p_limit_u)) == 0) {
        owner.SetInteriorLimitUpOffset(*(int32_t*)(addr + 4));
        f[5] = true;
      }
      if (!f[6] && memcmp((void*)addr, p_limit_d, sizeof(p_limit_d)) == 0) {
        owner.SetInteriorLimitDownOffset(*(int32_t*)(addr + 4));
        f[6] = true;
      }
      if (!f[7] && memcmp((void*)addr, p_default_ud, sizeof(p_default_ud)) == 0) {
        owner.SetInteriorMouseUDDefaultOffset(*(int32_t*)(addr + 2));
        f[7] = true;
      }
  ...
  // --- Find offsets within UpdateInteriorCameraOrientation ---
  {
    ...
    for (int i = 0; i < 2048; ++i) {
      ...
      if (!found_yaw && memcmp((void*)addr, yaw_pattern, sizeof(yaw_pattern)) == 0) {
        owner.SetInteriorYawOffset(*(int32_t*)(addr + 4));
        found_yaw = true;
      }
      if (!found_pitch && memcmp((void*)addr, pitch_pattern, sizeof(pitch_pattern)) == 0) {
        owner.SetInteriorPitchOffset(*(int32_t*)(addr + 4));
        found_pitch = true;
      }
  ...
  // FOV offsets are shared and found by InteriorCameraDataFinder, so we don't search for them here.

  m_isReady = all_found;
  if (all_found) {
    logger->Info("Successfully found all Interior Camera offsets.");
  } else {
    logger->Error("Failed to find one or more Interior Camera offsets.");
  }
  return all_found;
}
