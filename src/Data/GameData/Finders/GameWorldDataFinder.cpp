...
bool WorldDataFinder::TryFindOffsets(GameWorldService& owner) {
     ...
     */
    const std::vector<std::string> mapScaleChain = {
        "F3 [40-4F] 0F [10-59] [80-BF] ?? ?? ?? ??", 
        "[40-4F] [C1-D1] [E8-EF]"
    };
 ...
 /*
     ...
     */
    const std::vector<std::string> warpChain = {
        "F3 41 0F 10 [80-BF] ?? ?? ?? ??", 
        "F3 [40-4F] 0F 10 0D ?? ?? ?? ??"
    };
    ...
    if (addr) {
        ...
        if (Utils::PatternFinder::IsSaneOffset(trafficOff) && 
            Utils::PatternFinder::IsSaneOffset(simOff) && 
            Utils::PatternFinder::IsSaneOffset(globalOff)) 
        {
            owner.SetTrafficHaltOffset(trafficOff);
            owner.SetSimulationHaltOffset(simOff);
            owner.SetGlobalHaltOffset(globalOff);
            logger->Debug("5. [OFFSET: Engine Halt] Found Global: 0x{:X}, Sim: 0x{:X}, Traffic: 0x{:X}", 
                globalOff, simOff, trafficOff);
        } else {
            logger->Error("5. [OFFSET: Engine Halt] One or more offsets (0x{:X}, 0x{:X}, 0x{:X}) are insane.", 
                globalOff, simOff, trafficOff);
            all_found = false;
        }
    } else {
