...
bool SessionDataFinder::TryFindOffsets(GameObjectSessionService& owner) {
  ...
  if (addrSelectProfile) {
    ...
    logger->Error("1. Failed to find SelectProfile via log string.");
  }
  ...
  if (addrMpFunc) {
    ...
    if (addrMgr) {
      ...
      logger->Error("2. Failed to find Session Manager MOV instruction in MP function.");
    }
  } else {
    logger->Error("2. Failed to find target MP session function.");
  }
  ...
  if (addrLogStr) {
    ...
    if (addrName) {
      ...
      logger->Error("3.1 [OFFSET: Display Name] Failed to find MOV instruction.");
    }
    ...
    if (addrPathFunc) {
      ...
      if (addrTypeMov) {
        ...
        logger->Error("3.2 [OFFSET: Profile Type] Failed to find MOV instruction in Path function.");
      }
    } else {
      logger->Error("3.2 [OFFSET: Profile Type] Failed to find SetProfileBasePath function.");
    }
  } else {
