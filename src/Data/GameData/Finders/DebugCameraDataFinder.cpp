...
bool DebugCameraDataFinder::TryFindOffsets(GameDataCameraService& owner) {
  ...
  if (pfnSetMode) {
    ...
    if (addrModeSet) {
      ...
      if (Utils::PatternFinder::IsSaneOffset(offset)) {
        ...
        logger->Error("Debug Mode offset (via SetMode) INVALID (0x{:X})", offset);
      }
    } else {
      logger->Warn("FAILED to find Debug Camera Mode offset anchor inside SetDebugCameraMode");
    }
      ...
      if (base_ptr && pfnGetAndCache) {
        ...
        if (sig_off) {
          ...
          if (Utils::PatternFinder::IsSaneOffset(offset)) {
            ...
            logger->Error("CVar Value Offset INVALID (0x{:X})", offset);
            all_found = false;
          }
        } else {
          logger->Error("FAILED to find CVar Value Offset epilogue inside GetAndCacheValue");
          all_found = false;
        }
      } else {
        logger->Error("FAILED to resolve CVar Object or Cache Function RIP from SetDebugCameraMode");
        all_found = false;
      }
    } else {
      logger->Error("FAILED to find CVar discovery sequence inside SetDebugCameraMode");
      all_found = false;
    }
  } else {
    ...
    if (!xrefs.empty()) {
      ...
      if (addrUI) {
        ...
        if (Utils::PatternFinder::IsSaneOffset(off)) {
          ...
          logger->Error("Game UI offset INVALID (0x{:X})", off);
          all_found = false;
        }
      } else {
        logger->Error("FAILED to find Game UI Visible anchor via backward search from 0x{:X}", xrefs[0]);
        all_found = false;
      }
      ...
      if (posLockCall) {
        ...
        if (Utils::PatternFinder::IsSaneOffset(off) && pfn) {
          ...
          logger->Error("SetPositionLock data INVALID (pfn: 0x{:X})", pfn);
          all_found = false;
        }
      } else {
        logger->Error("FAILED to find SetPositionLock call site");
        all_found = false;
      }
      ...
      if (rotLockCall) {
        ...
        if (Utils::PatternFinder::IsSaneOffset(off) && pfn) {
          ...
          logger->Error("SetRotationLock data INVALID (pfn: 0x{:X})", pfn);
          all_found = false;
        }
      } else {
        logger->Error("FAILED to find SetRotationLock call site");
        all_found = false;
      }
      ...
      if (orbitCall) {
        ...
        if (Utils::PatternFinder::IsSaneOffset(off) && pfn) {
          ...
          logger->Error("SetOrbitMode data INVALID (pfn: 0x{:X})", pfn);
          all_found = false;
        }
      } else {
        logger->Error("FAILED to find SetOrbitMode call site");
        all_found = false;
      }
      ...
      if (pfnSetHudVis && pfnSetHudPos) {
        ...
        if (fnStart && fnEnd) {
          ...
          if (owner.GetHudVisibleOffset() == 0) {
            logger->Error("FAILED to find HUD Visible offset in HandleInput");
            all_found = false;
          }
          ...
          if (owner.GetHudPositionOffset() == 0) {
            logger->Error("FAILED to find HUD Position offset in HandleInput");
            all_found = false;
          }
        }
      }
    } else {
      logger->Error("FAILED to find XREFs for ROLL_SPEED string");
      all_found = false;
    }
  } else {
    logger->Error("FAILED to find ROLL_SPEED string anchor '{}'", DEBUG_ROLL_SPEED_STR);
    all_found = false;
  }
  ...
  if (pfnSelected) {
    ...
    logger->Error("FAILED to find SetSelectedActor via string anchor '{}'", SELECTED_ACTOR_STR);
    all_found = false;
  }
  ...
  if (posLockUsage) {
    ...
    if (addrLock) {
      ...
      if (Utils::PatternFinder::IsSaneOffset(off)) {
        ...
        logger->Error("Pos/Rot Lock offset INVALID (0x{:X})", off);
        all_found = false;
      }
    } else {
      logger->Error("FAILED to find CMP anchor backward from '{}' usage", POS_LOCK_STR);
      all_found = false;
    }
  } else {
    logger->Error("FAILED to find local usage of string anchor '{}'", POS_LOCK_STR);
    all_found = false;
  }
  ...
  if (orbitUsage) {
    ...
    if (addrOrbit) {
      ...
      if (Utils::PatternFinder::IsSaneOffset(off)) {
        ...
        logger->Error("Orbit offset INVALID (0x{:X})", off);
        all_found = false;
      }
    } else {
      logger->Error("FAILED to find CMP anchor backward from '{}' usage", ORBIT_STR);
      all_found = false;
    }
  } else {
    logger->Error("FAILED to find local usage of string anchor '{}'", ORBIT_STR);
    all_found = false;
  }
  ...
  if (orbitSpeedUsage) {
    ...
    if (addr) {
      ...
      if (Utils::PatternFinder::IsSaneOffset(offSel) && Utils::PatternFinder::IsSaneOffset(offSpd)) {
        ...
        logger->Error("SelectedObj/Speed offset INVALID (0x{:X}/0x{:X})", offSel, offSpd);
        all_found = false;
      }
    } else {
      logger->Error("FAILED to find MOV anchor backward from '{}' usage", ORBIT_SPEED_STR);
      all_found = false;
    }
  } else {
    logger->Error("FAILED to find local usage of string anchor '{}'", ORBIT_SPEED_STR);
    all_found = false;
  }
  ...
  if (actorUsage) {
    ...
    if (addrHover) {
      ...
      if (Utils::PatternFinder::IsSaneOffset(off)) {
        ...
        logger->Error("Hovered Object offset INVALID (0x{:X})", off);
        all_found = false;
      }
    } else {
      logger->Error("FAILED to find CMP anchor backward from '{}' usage", ACTOR_STR);
      all_found = false;
    }
  } else {
    logger->Error("FAILED to find local usage of string anchor '{}'", ACTOR_STR);
    all_found = false;
  }
