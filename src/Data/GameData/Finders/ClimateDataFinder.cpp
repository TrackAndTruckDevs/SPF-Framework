...
bool ClimateDataFinder::TryFindOffsets(ClimateService& owner) {
  ...
  /*
   * 1. [DATA: Global Managers] (Environment)
   ...
   * We locate it via the log string "Restarting environment transition, possibly upcoming blending errors.".
   * This is how this line is modified for two functions.
   * s_Restarting_environment_transitio_142127b60 XREF[2]: SetWeather:1404df4dc(*), SetClimate:1404df709(*)
   *
   * To disambiguate from other functions using the same string, we use a tight context search.
   *
   * /--- Ghidra:(amtrucks_1_60.exe) Fun:(SetWeather[1404df3e0]) ---/
   * 1404df4dc 48 8d 0d ...  LEA RCX, [s_Restarting_environment_transitio...]
   * ...
   * 1404df4e8  8B 8B 78 3E 00 00             MOV ECX,dword ptr [RBX + 0x3e78]
   * 1404df4ee  B8 6D C1 16 6C                MOV EAX,0x6c16c16d
   * 1404df4f3  F3 0F 10 83 7C 3E 00 00       MOVSS XMM0,dword ptr [RBX + 0x3e7c
   */
  ...
  if (!SetWeatherFn) {
    ...
    logger->Info("4. [CALL: SetWeather] Found at 0x{:X}", SetWeatherFn);
  }
  ...
  if (addr) {
    ...
    if (Utils::PatternFinder::IsSaneOffset(weatherModeOff) && Utils::PatternFinder::IsSaneOffset(weatherTargetOff)) {
      ...
      logger->Debug("4.1 [OFFSET: Weather Indexes] Found Current: 0x{:X}, Target: 0x{:X}", weatherModeOff, weatherTargetOff);
    }
    ...
    uintptr_t addrDur = Utils::PatternFinder::FindChain(durChain, 20, SetWeatherFn);
    if (addrDur) {
   ...
   */
  // const char* SET_CLIMATE_SIG = "48 89 5C 24 ?? 48 89 [68-74] 24 ?? 57 48 83 EC [30-50] 0F 29 [70-7C] 24 ?? 0f ?? ?? 41";
