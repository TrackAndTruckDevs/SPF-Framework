...
bool BumperCameraDataFinder::TryFindOffsets(GameDataCameraService& owner) {
  ...
  logger->Info("--- STARTING BUMPER CAMERA OFFSET SEARCH (Reflection) ---");

  const char* CLASS_NAME_BUMPER = "vehicle_bumper_camera";
  ...
  // --- Step 1: Find vehicle_bumper_camera SII Attributes ---

  uintptr_t cameraOff = getAttr(CLASS_NAME_BUMPER, "camera_offset");
  if (cameraOff) {
    owner.SetBumperOffsetXOffset(static_cast<intptr_t>(cameraOff));
    owner.SetBumperOffsetYOffset(static_cast<intptr_t>(cameraOff + 4));
    owner.SetBumperOffsetZOffset(static_cast<intptr_t>(cameraOff + 8));
  }
  ...
  m_isReady =
    all_found && (owner.GetBumperOffsetXOffset() != 0 && owner.GetCameraFovOffset() != 0 && owner.GetShakeAnimStepOffset() != 0 && owner.GetShakeAnimScaleMinOffset() != 0 && owner.GetShakeAnimScaleMaxOffset() != 0 && owner.GetShakeAnimOffset() != 0);

  auto end = std::chrono::high_resolution_clock::now();
  auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

  if (m_isReady) {
    logger->Info("--- BUMPER CAMERA OFFSETS FOUND. BumperCameraDataFinder is ready. ({} ms) ---", duration);
  } else {
    logger->Error("FAILED to initialize one or more Bumper Camera offsets. ({} ms)", duration);
  }
