...
void GameCameraWheel::OnActivate() {
  auto logger = Logging::LoggerFactory::GetInstance().GetLogger("GameCameraWheel");
  logger->Info("Activating Wheel Camera.");
...
void GameCameraWheel::OnDeactivate() {
  auto logger = Logging::LoggerFactory::GetInstance().GetLogger("GameCameraWheel");
  logger->Info("Deactivating Wheel Camera.");
...
void GameCameraWheel::SetOffset(float x, float y, float z) {
  ...
  auto x_offset = gameData.GetWheelOffsetXOffset();
  auto y_offset = gameData.GetWheelOffsetYOffset();
  auto z_offset = gameData.GetWheelOffsetZOffset();
...
void GameCameraWheel::SetFov(float fov) {
  if (!m_pCameraObject) return;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  auto& hooks = Hooks::CameraHooks::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
...
void GameCameraWheel::StoreDefaultState() {
...
void GameCameraWheel::ResetToDefaults() {
  if (!m_defaultsSaved || !m_pCameraObject) return;
...
bool GameCameraWheel::GetOffset(float* out_x, float* out_y, float* out_z) const {
...
bool GameCameraWheel::GetFov(float* out_fov) const {
...
bool GameCameraWheel::GetFinalFov(float* out_horiz, float* out_vert) const {
...
bool GameCameraWheel::GetShakeAnimStep(float* out_val) const {
...
bool GameCameraWheel::GetShakeAnimScaleMin(float* out_val) const {
  if (!out_val || !m_pCameraObject) return false;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
...
bool GameCameraWheel::GetShakeAnimScaleMax(float* out_val) const {
  if (!out_val || !m_pCameraObject) return false;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
...
size_t GameCameraWheel::GetShakeAnimCount() const {
...
void GameCameraWheel::GetShakeAnim(size_t index, float& x, float& y, float& z) const {
  if (!m_pCameraObject) return;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
  auto off = gameData.GetShakeAnimOffset();
...
void GameCameraWheel::SetShakeAnimStep(float val) {
  if (!m_pCameraObject) return;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
  auto off = gameData.GetShakeAnimStepOffset();
...
void GameCameraWheel::SetShakeAnimScaleMin(float val) {
  if (!m_pCameraObject) return;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
  auto off = gameData.GetShakeAnimScaleMinOffset();
...
void GameCameraWheel::SetShakeAnimScaleMax(float val) {
  if (!m_pCameraObject) return;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
  auto off = gameData.GetShakeAnimScaleMaxOffset();
...
void GameCameraWheel::SetShakeAnim(size_t index, float x, float y, float z) {
  if (!m_pCameraObject) return;
  auto& gameData = Data::GameData::GameDataCameraService::GetInstance();
  uintptr_t pCam = reinterpret_cast<uintptr_t>(m_pCameraObject);
  auto off = gameData.GetShakeAnimOffset();
