...
void GameCameraTV::SetPrefabUplift(float x, float y, float z) {
  ...
  auto x_offset = gameData.GetTVPrefabUpliftXOffset();
  auto y_offset = gameData.GetTVPrefabUpliftYOffset();
  auto z_offset = gameData.GetTVPrefabUpliftZOffset();
...
void GameCameraTV::SetRoadUplift(float x, float y, float z) {
  ...
  auto x_offset = gameData.GetTVRoadUpliftXOffset();
  auto y_offset = gameData.GetTVRoadUpliftYOffset();
  auto z_offset = gameData.GetTVRoadUpliftZOffset();
...
void GameCameraTV::StoreDefaultState() {
  ...
  float max_distance_val;
  if (GetMaxDistance(&max_distance_val)) m_defaultCameraData.max_distance = max_distance_val;

  float prefab_x, prefab_y, prefab_z;
  if (GetPrefabUplift(&prefab_x, &prefab_y, &prefab_z)) {
    m_defaultCameraData.prefab_uplift_x = prefab_x;
    m_defaultCameraData.prefab_uplift_y = prefab_y;
    m_defaultCameraData.prefab_uplift_z = prefab_z;
  }

  float road_x, road_y, road_z;
  if (GetRoadUplift(&road_x, &road_y, &road_z)) {
    m_defaultCameraData.road_uplift_x = road_x;
    m_defaultCameraData.road_uplift_y = road_y;
    m_defaultCameraData.road_uplift_z = road_z;
  }
...
bool GameCameraTV::GetMaxDistance(float* out_max_distance) const {
  if (!out_max_distance) return false;
