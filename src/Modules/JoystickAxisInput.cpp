...
uint32_t JoystickAxisInput::GetHardwareCode() const {
    return 0x04010000 | static_cast<uint32_t>(m_axisIndex);
}

bool JoystickAxisInput::IsActive(const std::set<uint32_t>& pressedCodes) const {
...
bool JoystickAxisInput::InvolvesHardwareCode(uint32_t code) const {
...
std::string JoystickAxisInput::GetDisplayName() const {
    return fmt::format("Joystick Axis {}", m_axisIndex);
}

bool JoystickAxisInput::IsValid() const {
    return m_axisIndex >= 0 && m_axisIndex < 16;
}
