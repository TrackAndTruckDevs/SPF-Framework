...
MouseAxisInput::MouseAxisInput(const nlohmann::ordered_json& config) {
    ...
    m_rangeMin = config.value("range_min", -100.0f);
    m_rangeMax = config.value("range_max", 100.0f);
}

bool MouseAxisInput::IsTriggeredBy(const Input::MouseMoveEvent& event) const {
    return false; // Axes use GetValue
}

uint32_t MouseAxisInput::GetHardwareCode() const {
    return 0x03010000 | static_cast<uint32_t>(m_axisIndex);
}

bool MouseAxisInput::IsActive(const std::set<uint32_t>& pressedCodes) const {
    return pressedCodes.count(GetHardwareCode()) > 0;
}

float MouseAxisInput::GetValue(const std::set<uint32_t>& pressedHardwareCodes, const std::map<uint32_t, float>& axisValues) const {
    ...
    // 4. Apply Curves (Sensitivity is now handled at accumulation stage)
    processed = processed; // Sensitivity moved to InputManager for relative devices
    ...
    //     logger->Debug("[Mouse] Axis {}: Raw={:.4f} -> Final={:.4f} (Side={}, Mode={})", 
...
bool MouseAxisInput::InvolvesHardwareCode(uint32_t code) const {
    return GetHardwareCode() == code;
}

bool MouseAxisInput::IsSameAs(const IBindableInput& other) const {
    if (other.GetType() != InputType::MouseAxis) return false;
    const auto& otherInput = static_cast<const MouseAxisInput&>(other);
    return m_axisIndex == otherInput.m_axisIndex;
}

nlohmann::ordered_json MouseAxisInput::ToJson() const {
...
std::string MouseAxisInput::GetDisplayName() const {
    switch (m_axisIndex) {
        case 0: return "Mouse X";
        case 1: return "Mouse Y";
        case 2: return "Mouse Wheel";
        default: return fmt::format("Mouse Axis {}", m_axisIndex);
    }
}

bool MouseAxisInput::IsValid() const {
    return m_axisIndex >= 0 && m_axisIndex <= 2;
}
