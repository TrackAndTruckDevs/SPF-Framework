...
uint32_t MouseAxisInput::GetHardwareCode() const { return 0x03010000 | static_cast<uint32_t>(m_axisIndex); }

bool MouseAxisInput::IsActive(const std::set<uint32_t>& pressedCodes) const { return pressedCodes.count(GetHardwareCode()) > 0; }
...
bool MouseAxisInput::InvolvesHardwareCode(uint32_t code) const { return GetHardwareCode() == code; }
...
std::string MouseAxisInput::GetDisplayName() const {
  switch (m_axisIndex) {
    ...
    case 2:
      return "Mouse Wheel";
    default:
      return fmt::format("Mouse Axis {}", m_axisIndex);
  }
}
