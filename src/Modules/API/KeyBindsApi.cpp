...
void KeyBindsApi::Kbind_Register_Ex(SPF_KeyBinds_Handle* h, const char* actionName, SPF_Keybind_Callback_Ex callback, void* user_data) {
  ...
  auto& pm = PluginManager::GetInstance();
  if (pm.GetKeyBindsManager()) {
    pm.GetKeyBindsManager()->RegisterActionEx(fullActionName, [callback](const std::string& id, void* data) { callback(id.c_str(), data); }, user_data);
  }
}
...
SPF_BindingType KeyBindsApi::Kbind_GetBindingType(SPF_KeyBinds_Handle* h, const char* actionName, int index) {
  ...
  switch (binding->Input->GetType()) {
    case Modules::InputType::Keyboard:
      return SPF_BINDING_KEYBOARD;
    case Modules::InputType::Gamepad:
      return SPF_BINDING_GAMEPAD;
    case Modules::InputType::Mouse:
      return SPF_BINDING_MOUSE;
    case Modules::InputType::Joystick:
      return SPF_BINDING_JOYSTICK;
    case Modules::InputType::Chord:
      return SPF_BINDING_CHORD;
    case Modules::InputType::GamepadAxis:
      return SPF_BINDING_GAMEPAD_AXIS;
    case Modules::InputType::MouseAxis:
      return SPF_BINDING_MOUSE_AXIS;
    case Modules::InputType::JoystickAxis:
      return SPF_BINDING_JOYSTICK_AXIS;
    default:
      return SPF_BINDING_UNKNOWN;
  }
}
