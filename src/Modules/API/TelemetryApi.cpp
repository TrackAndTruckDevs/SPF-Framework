...
const char* GameToString(Telemetry::Game game) {
  switch (game) {
    case Telemetry::Game::ETS2:
      return "ETS2";
    case Telemetry::Game::ATS:
      return "ATS";
    case Telemetry::Game::Unknown:
      return "Unknown";
    default:
      return "Invalid";
  }
...
SPF_Telemetry_Handle* TelemetryApi::Tel_GetContext(const char* pluginName) {
...
void TelemetryApi::Tel_GetGameState(SPF_Telemetry_Handle* h, SPF_GameState* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  ...
  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_GameState)));
}

void TelemetryApi::Tel_GetTimestamps(SPF_Telemetry_Handle* h, SPF_Timestamps* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;
  ...
  c_data.paused_simulation = cpp_data.paused_simulation;

  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_Timestamps)));
}

void TelemetryApi::Tel_GetCommonData(SPF_Telemetry_Handle* h, SPF_CommonData* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetCommonData();
  SPF_CommonData c_data;
  memset(&c_data, 0, sizeof(SPF_CommonData));
...
void TelemetryApi::Tel_GetTruckConstants(SPF_Telemetry_Handle* h, SPF_TruckConstants* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetTruckConstants();
  SPF_TruckConstants c_data;
  memset(&c_data, 0, sizeof(SPF_TruckConstants));
  ...
  if (reverse_gears_to_copy > 0) {
    memcpy(c_data.gear_ratios_reverse, cpp_data.gear_ratios_reverse.data(), reverse_gears_to_copy * sizeof(float));
  }

  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_TruckConstants)));
}

void TelemetryApi::Tel_GetTruckData(SPF_Telemetry_Handle* h, SPF_TruckData* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;
  ...
  memset(&c_data, 0, sizeof(SPF_TruckData));
  ...
  c_data.head_offset = {{cpp_data.head_offset.position.x, cpp_data.head_offset.position.y, cpp_data.head_offset.position.z}, {cpp_data.head_offset.orientation.heading, cpp_data.head_offset.orientation.pitch, cpp_data.head_offset.orientation.roll}};
  ...
  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_TruckData)));
}

void TelemetryApi::Tel_GetTrailers(SPF_Telemetry_Handle* h, SPF_Trailer* out_trailers, size_t struct_size, uint32_t* in_out_count) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_trailers || !in_out_count || struct_size == 0 || !pm.GetTelemetryService()) return;
  ...
  for (uint32_t i = 0; i < trailers_to_copy; ++i) {
    ...
    SPF_Trailer c_trailer;
    memset(&c_trailer, 0, sizeof(SPF_Trailer));
    ...
    // Use pointer arithmetic to reach the current element's position in the plugin's buffer
    char* target_ptr = reinterpret_cast<char*>(out_trailers) + (i * struct_size);
    memcpy(target_ptr, &c_trailer, (std::min)(struct_size, sizeof(SPF_Trailer)));
  }

  *in_out_count = trailers_to_copy;
}

void TelemetryApi::Tel_GetJobConstants(SPF_Telemetry_Handle* h, SPF_JobConstants* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetJobConstants();
  SPF_JobConstants c_data;
  memset(&c_data, 0, sizeof(SPF_JobConstants));
...
void TelemetryApi::Tel_GetJobData(SPF_Telemetry_Handle* h, SPF_JobData* out_data, size_t struct_size) {
  ...
  SPF_JobData c_data;
  memset(&c_data, 0, sizeof(SPF_JobData));

  c_data.on_job = cpp_data.on_job;
  c_data.cargo_damage = cpp_data.cargo_damage;
  c_data.remaining_delivery_minutes = cpp_data.remaining_delivery_minutes;

  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_JobData)));
}

void TelemetryApi::Tel_GetNavigationData(SPF_Telemetry_Handle* h, SPF_NavigationData* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetNavigationData();
  SPF_NavigationData c_data;
  memset(&c_data, 0, sizeof(SPF_NavigationData));
  ...
  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_NavigationData)));
}

void TelemetryApi::Tel_GetControls(SPF_Telemetry_Handle* h, SPF_Controls* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetControls();
  SPF_Controls c_data;
  memset(&c_data, 0, sizeof(SPF_Controls));
  ...
  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_Controls)));
}

void TelemetryApi::Tel_GetSpecialEvents(SPF_Telemetry_Handle* h, SPF_SpecialEvents* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetSpecialEvents();
  SPF_SpecialEvents c_data;
  memset(&c_data, 0, sizeof(SPF_SpecialEvents));
...
void TelemetryApi::Tel_GetGameplayEvents(SPF_Telemetry_Handle* h, SPF_GameplayEvents* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetGameplayEvents();
  SPF_GameplayEvents c_data;
  memset(&c_data, 0, sizeof(SPF_GameplayEvents));
...
void TelemetryApi::Tel_GetGearboxConstants(SPF_Telemetry_Handle* h, SPF_GearboxConstants* out_data, size_t struct_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_data || struct_size == 0 || !pm.GetTelemetryService()) return;

  const auto& cpp_data = pm.GetTelemetryService()->GetGearboxConstants();
  SPF_GearboxConstants c_data;
  memset(&c_data, 0, sizeof(SPF_GearboxConstants));
  ...
  memcpy(out_data, &c_data, (std::min)(struct_size, sizeof(SPF_GearboxConstants)));
}

int TelemetryApi::Tel_GetLastGameplayEventId(SPF_Telemetry_Handle* h, char* out_buffer, int buffer_size) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !out_buffer || buffer_size <= 0 || !pm.GetTelemetryService()) return 0;

  const auto& event_id = pm.GetTelemetryService()->GetLastGameplayEventId();
  if (static_cast<int>(event_id.length()) < buffer_size) {
    strcpy_s(out_buffer, buffer_size, event_id.c_str());
    return static_cast<int>(event_id.length());
  } else {
    *out_buffer = '\0';
    return static_cast<int>(event_id.length() + 1);  // Return required size
  }
}
...
void TelemetryApi::FillTelemetryApi(SPF_Telemetry_API* api) {
  ...
  api->Tel_GetContext = &TelemetryApi::Tel_GetContext;
  api->Tel_GetGameState = &TelemetryApi::Tel_GetGameState;
  api->Tel_GetTimestamps = &TelemetryApi::Tel_GetTimestamps;
  api->Tel_GetCommonData = &TelemetryApi::Tel_GetCommonData;
  api->Tel_GetTruckConstants = &TelemetryApi::Tel_GetTruckConstants;
  ...
  api->Tel_GetSpecialEvents = &TelemetryApi::Tel_GetSpecialEvents;
  api->Tel_GetGameplayEvents = &TelemetryApi::Tel_GetGameplayEvents;
  api->Tel_GetGearboxConstants = &TelemetryApi::Tel_GetGearboxConstants;
  api->Tel_GetLastGameplayEventId = &TelemetryApi::Tel_GetLastGameplayEventId;

  // Assign RAII-based event subscription functions
  api->Tel_RegisterForGameState = &TelemetryApi::Tel_RegisterForGameState;
  api->Tel_RegisterForTimestamps = &TelemetryApi::Tel_RegisterForTimestamps;
  api->Tel_RegisterForCommonData = &TelemetryApi::Tel_RegisterForCommonData;
  api->Tel_RegisterForTruckConstants = &TelemetryApi::Tel_RegisterForTruckConstants;
  api->Tel_RegisterForTrailerConstants = &TelemetryApi::Tel_RegisterForTrailerConstants;
...
SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForGameState(SPF_Telemetry_Handle* h, SPF_Telemetry_GameState_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::GameState>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::GameState& cpp_data, void* ud) { TelemetryApi::InvokeGameStateCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::GameState>>(pm.GetTelemetryService()->GetGameStateSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForTimestamps(SPF_Telemetry_Handle* h, SPF_Telemetry_Timestamps_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::Timestamps>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::Timestamps& cpp_data, void* ud) { TelemetryApi::InvokeTimestampsCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::Timestamps>>(pm.GetTelemetryService()->GetTimestampsSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForCommonData(SPF_Telemetry_Handle* h, SPF_Telemetry_CommonData_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::CommonData>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::CommonData& cpp_data, void* ud) { TelemetryApi::InvokeCommonDataCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::CommonData>>(pm.GetTelemetryService()->GetCommonDataSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForTruckConstants(SPF_Telemetry_Handle* h, SPF_Telemetry_TruckConstants_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::TruckConstants>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::TruckConstants& cpp_data, void* ud) { TelemetryApi::InvokeTruckConstantsCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::TruckConstants>>(pm.GetTelemetryService()->GetTruckConstantsSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForTrailerConstants(SPF_Telemetry_Handle* h, SPF_Telemetry_TrailerConstants_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::TrailerConstants>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::TrailerConstants& cpp_data, void* ud) { TelemetryApi::InvokeTrailerConstantsCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::TrailerConstants>>(pm.GetTelemetryService()->GetTrailerConstantsSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForTruckData(SPF_Telemetry_Handle* h, SPF_Telemetry_TruckData_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::TruckData>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::TruckData& cpp_data, void* ud) { TelemetryApi::InvokeTruckDataCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::TruckData>>(pm.GetTelemetryService()->GetTruckDataSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForTrailers(SPF_Telemetry_Handle* h, SPF_Telemetry_Trailers_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<std::vector<SPF::Telemetry::SCS::Trailer>>::InvokerFunction invoker = [callback](const std::vector<SPF::Telemetry::SCS::Trailer>& cpp_data, void* ud) { TelemetryApi::InvokeTrailersCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<std::vector<SPF::Telemetry::SCS::Trailer>>>(pm.GetTelemetryService()->GetTrailersSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForJobConstants(SPF_Telemetry_Handle* h, SPF_Telemetry_JobConstants_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
...
SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForJobData(SPF_Telemetry_Handle* h, SPF_Telemetry_JobData_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::JobData>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::JobData& cpp_data, void* ud) { TelemetryApi::InvokeJobDataCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::JobData>>(pm.GetTelemetryService()->GetJobDataSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForNavigationData(SPF_Telemetry_Handle* h, SPF_Telemetry_NavigationData_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::NavigationData>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::NavigationData& cpp_data, void* ud) { TelemetryApi::InvokeNavigationDataCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::NavigationData>>(pm.GetTelemetryService()->GetNavigationDataSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForControls(SPF_Telemetry_Handle* h, SPF_Telemetry_Controls_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::Controls>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::Controls& cpp_data, void* ud) { TelemetryApi::InvokeControlsCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::Controls>>(pm.GetTelemetryService()->GetControlsSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForSpecialEvents(SPF_Telemetry_Handle* h, SPF_Telemetry_SpecialEvents_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  SubscriptionHandler<SPF::Telemetry::SCS::SpecialEvents>::InvokerFunction invoker = [callback](const SPF::Telemetry::SCS::SpecialEvents& cpp_data, void* ud) { TelemetryApi::InvokeSpecialEventsCallback(cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<SubscriptionHandler<SPF::Telemetry::SCS::SpecialEvents>>(pm.GetTelemetryService()->GetSpecialEventsSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForGameplayEvents(SPF_Telemetry_Handle* h, SPF_Telemetry_GameplayEvents_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
  ...
  GameplayEventSubscriptionHandler::InvokerFunction invoker = [callback](const char* event_id, const SPF::Telemetry::SCS::GameplayEvents& cpp_data, void* ud) { TelemetryApi::InvokeGameplayEventsCallback(event_id, cpp_data, callback, ud); };

  telemetryHandle->m_subscriptionHandlers.emplace_back(std::make_unique<GameplayEventSubscriptionHandler>(pm.GetTelemetryService()->GetGameplayEventsSignal(), invoker, user_data));
  return reinterpret_cast<SPF_Telemetry_Callback_Handle*>(telemetryHandle->m_subscriptionHandlers.back().get());
}

SPF_Telemetry_Callback_Handle* TelemetryApi::Tel_RegisterForGearboxConstants(SPF_Telemetry_Handle* h, SPF_Telemetry_GearboxConstants_Callback callback, void* user_data) {
  auto& pm = PluginManager::GetInstance();
  if (!h || !callback || !pm.GetTelemetryService()) return nullptr;
