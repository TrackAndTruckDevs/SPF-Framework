...
void UIApi::UI_RegisterDrawCallback(const char* pluginName, const char* windowId, SPF_DrawCallback drawCallback, void* user_data) { UI_RegisterDrawCallbackWithFlags(pluginName, windowId, drawCallback, user_data, SPF_WINDOW_FLAG_NONE); }
...
void UIApi::UI_SetAllocatorFunctions(void* (*alloc_func)(size_t sz, void* user_data), void (*free_func)(void* ptr, void* user_data), void* user_data) { ImGui::SetAllocatorFunctions(alloc_func, free_func, user_data); }
...
bool UIApi::UI_IsMouseHoveringRect(float min_x, float min_y, float max_x, float max_y, bool clip) { return ImGui::IsMouseHoveringRect({min_x, min_y}, {max_x, max_y}, clip); }
...
bool UIApi::UI_BeginChild(const char* str_id, float size_x, float size_y, bool border, SPF_WindowFlags flags) { return ImGui::BeginChild(str_id, {size_x, size_y}, border, (ImGuiWindowFlags)flags); }
...
void UIApi::UI_TextUnformatted(const char* text) {
  if (text) ImGui::TextUnformatted(text);
}
...
void UIApi::UI_TextColored(float r, float g, float b, float a, const char* text) {
  if (text) ImGui::TextColored({r, g, b, a}, "%s", text);
}
...
bool UIApi::UI_ImageButton(const char* str_id, void* user_texture_id, float width, float height) { return ImGui::ImageButton(str_id, user_texture_id, {width, height}); }
...
bool UIApi::UI_DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, SPF_SliderFlags flags) {
...
bool UIApi::UI_DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max, SPF_SliderFlags flags) {
...
bool UIApi::UI_DragScalar(const char* label, SPF_DataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, SPF_SliderFlags flags) {
  return ImGui::DragScalar(label, (ImGuiDataType)data_type, p_data, v_speed, p_min, p_max, format, (ImGuiSliderFlags)flags);
}
bool UIApi::UI_DragScalarN(const char* label, SPF_DataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, SPF_SliderFlags flags) {
...
bool UIApi::UI_SliderAngle(const char* label, float* v_rad, float v_degrees_min, float v_degrees_max, const char* format, SPF_SliderFlags flags) {
...
bool UIApi::UI_SliderScalar(const char* label, SPF_DataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, SPF_SliderFlags flags) {
  return ImGui::SliderScalar(label, (ImGuiDataType)data_type, p_data, p_min, p_max, format, (ImGuiSliderFlags)flags);
}
bool UIApi::UI_SliderScalarN(const char* label, SPF_DataType data_type, void* p_data, int components, const void* p_min, const void* p_max, const char* format, SPF_SliderFlags flags) {
  return ImGui::SliderScalarN(label, (ImGuiDataType)data_type, p_data, components, p_min, p_max, format, (ImGuiSliderFlags)flags);
}
bool UIApi::UI_VSliderFloat(const char* label, float size_x, float size_y, float* v, float v_min, float v_max, const char* format, SPF_SliderFlags flags) {
  return ImGui::VSliderFloat(label, {size_x, size_y}, v, v_min, v_max, format, (ImGuiSliderFlags)flags);
}
...
bool UIApi::UI_VSliderScalar(const char* label, float size_x, float size_y, SPF_DataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, SPF_SliderFlags flags) {
  return ImGui::VSliderScalar(label, {size_x, size_y}, (ImGuiDataType)data_type, p_data, p_min, p_max, format, (ImGuiSliderFlags)flags);
}
...
bool UIApi::UI_InputScalar(const char* label, SPF_DataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, SPF_InputTextFlags flags) {
  return ImGui::InputScalar(label, (ImGuiDataType)data_type, p_data, p_step, p_step_fast, format, (ImGuiInputTextFlags)flags);
}
bool UIApi::UI_InputScalarN(const char* label, SPF_DataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, SPF_InputTextFlags flags) {
...
SPF_MultiSelectIO* UIApi::UI_BeginMultiSelect(SPF_MultiSelectFlags flags, int selection_size, int items_count) { return (SPF_MultiSelectIO*)ImGui::BeginMultiSelect((ImGuiMultiSelectFlags)flags, selection_size, items_count); }

SPF_MultiSelectIO* UIApi::UI_EndMultiSelect() { return (SPF_MultiSelectIO*)ImGui::EndMultiSelect(); }

void UIApi::UI_SetNextItemSelectionUserData(int64_t selection_user_data) { ImGui::SetNextItemSelectionUserData((ImGuiSelectionUserData)selection_user_data); }

bool UIApi::UI_IsItemToggledSelection() { return ImGui::IsItemToggledSelection(); }

void UIApi::UI_PlotLines(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, float graph_size_x, float graph_size_y, int stride) {
  ImGui::PlotLines(label, values, values_count, values_offset, overlay_text, scale_min, scale_max, {graph_size_x, graph_size_y}, stride);
}
void UIApi::UI_PlotHistogram(const char* label, const float* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, float graph_size_x, float graph_size_y, int stride) {
  ImGui::PlotHistogram(label, values, values_count, values_offset, overlay_text, scale_min, scale_max, {graph_size_x, graph_size_y}, stride);
}
...
bool UIApi::UI_BeginTable(const char* str_id, int columns_count, SPF_TableFlags flags, float outer_size_x, float outer_size_y, float inner_width) {
  return ImGui::BeginTable(str_id, columns_count, (ImGuiTableFlags)flags, {outer_size_x, outer_size_y}, inner_width);
}
...
void UIApi::UI_SetItemTooltip(const char* text) {
...
void UIApi::UI_PushFont(SPF_Font_Handle font_handle) {
  if (font_handle) ImGui::PushFont((ImFont*)font_handle);
}
...
void UIApi::UI_CalcTextSizeWithFont(SPF_Font font, float font_size, const char* text, float* out_w, float* out_h) {
  ...
  switch (font) {
    case SPF_FONT_BOLD:
      fontName = "bold";
      break;
    case SPF_FONT_ITALIC:
      fontName = "italic";
      break;
    case SPF_FONT_BOLD_ITALIC:
      fontName = "bold_italic";
      break;
    case SPF_FONT_MEDIUM:
      fontName = "medium";
      break;
    case SPF_FONT_MEDIUM_ITALIC:
      fontName = "medium_italic";
      break;
    case SPF_FONT_MONOSPACE:
      fontName = "monospace";
      break;
    case SPF_FONT_H1:
      fontName = "h1";
    ...
    case SPF_FONT_H3:
      fontName = "h3";
      break;
    case SPF_FONT_H1_LARGE_BOLD:
      fontName = "h1_large_bold";
      break;
    case SPF_FONT_REGULAR:
      break;
  }
...
void UIApi::UI_Style_SetColor(SPF_TextStyle_Handle handle, float r, float g, float b, float a) {
  if (handle) handle->style.Color({r, g, b, a});
}
void UIApi::UI_Style_SetHoverColor(SPF_TextStyle_Handle handle, float r, float g, float b, float a) {
  if (handle) handle->style.HoverColor({r, g, b, a});
}
void UIApi::UI_Style_SetActiveColor(SPF_TextStyle_Handle handle, float r, float g, float b, float a) {
  if (handle) handle->style.ActiveColor({r, g, b, a});
}
void UIApi::UI_Style_SetAlign(SPF_TextStyle_Handle handle, SPF_TextAlign align) {
  if (handle) handle->style.Align((SPF::UI::TextAlign)align);
}
...
void UIApi::UI_DrawList_PushClipRect(SPF_DrawList_Handle dl, float p_min_x, float p_min_y, float p_max_x, float p_max_y, bool intersect_with_current_clip_rect) {
  if (dl) ((ImDrawList*)dl)->PushClipRect({p_min_x, p_min_y}, {p_max_x, p_max_y}, intersect_with_current_clip_rect);
}
void UIApi::UI_DrawList_PushClipRectFullScreen(SPF_DrawList_Handle dl) {
  if (dl) ((ImDrawList*)dl)->PushClipRectFullScreen();
}
void UIApi::UI_DrawList_PopClipRect(SPF_DrawList_Handle dl) {
  if (dl) ((ImDrawList*)dl)->PopClipRect();
}
...
void UIApi::UI_DrawList_AddLine(SPF_DrawList_Handle dl, float p1_x, float p1_y, float p2_x, float p2_y, uint32_t col, float thickness) {
  if (dl) ((ImDrawList*)dl)->AddLine({p1_x, p1_y}, {p2_x, p2_y}, col, thickness);
}
void UIApi::UI_DrawList_AddRect(SPF_DrawList_Handle dl, float p_min_x, float p_min_y, float p_max_x, float p_max_y, uint32_t col, float rounding, SPF_DrawFlags flags, float thickness) {
  if (dl) ((ImDrawList*)dl)->AddRect({p_min_x, p_min_y}, {p_max_x, p_max_y}, col, rounding, (ImDrawFlags)flags, thickness);
}
void UIApi::UI_DrawList_AddRectFilled(SPF_DrawList_Handle dl, float p_min_x, float p_min_y, float p_max_x, float p_max_y, uint32_t col, float rounding, SPF_DrawFlags flags) {
  if (dl) ((ImDrawList*)dl)->AddRectFilled({p_min_x, p_min_y}, {p_max_x, p_max_y}, col, rounding, (ImDrawFlags)flags);
}
void UIApi::UI_DrawList_AddRectFilledMultiColor(SPF_DrawList_Handle dl, float p_min_x, float p_min_y, float p_max_x, float p_max_y, uint32_t col_upr_left, uint32_t col_upr_right, uint32_t col_bot_right, uint32_t col_bot_left) {
  if (dl) ((ImDrawList*)dl)->AddRectFilledMultiColor({p_min_x, p_min_y}, {p_max_x, p_max_y}, col_upr_left, col_upr_right, col_bot_right, col_bot_left);
}

void UIApi::UI_DrawList_AddQuad(SPF_DrawList_Handle dl, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, float p4_x, float p4_y, uint32_t col, float thickness) {
  if (dl) ((ImDrawList*)dl)->AddQuad({p1_x, p1_y}, {p2_x, p2_y}, {p3_x, p3_y}, {p4_x, p4_y}, col, thickness);
}
void UIApi::UI_DrawList_AddQuadFilled(SPF_DrawList_Handle dl, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, float p4_x, float p4_y, uint32_t col) {
  if (dl) ((ImDrawList*)dl)->AddQuadFilled({p1_x, p1_y}, {p2_x, p2_y}, {p3_x, p3_y}, {p4_x, p4_y}, col);
}
void UIApi::UI_DrawList_AddTriangle(SPF_DrawList_Handle dl, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, uint32_t col, float thickness) {
  if (dl) ((ImDrawList*)dl)->AddTriangle({p1_x, p1_y}, {p2_x, p2_y}, {p3_x, p3_y}, col, thickness);
}
void UIApi::UI_DrawList_AddTriangleFilled(SPF_DrawList_Handle dl, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, uint32_t col) {
  if (dl) ((ImDrawList*)dl)->AddTriangleFilled({p1_x, p1_y}, {p2_x, p2_y}, {p3_x, p3_y}, col);
}
...
void UIApi::UI_DrawList_AddTriangleFilledMultiColor(SPF_DrawList_Handle dl, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, uint32_t col1, uint32_t col2, uint32_t col3) {
  ...
  for (int i = 0; i < 3; i++) {
    int j = (i + 1) % 3;
    drawList->PrimWriteIdx(base_idx + i);
    drawList->PrimWriteIdx(base_idx + j);
    drawList->PrimWriteIdx(base_idx + 3 + j);
    drawList->PrimWriteIdx(base_idx + i);
    drawList->PrimWriteIdx(base_idx + 3 + j);
    drawList->PrimWriteIdx(base_idx + 3 + i);
  }
...
void UIApi::UI_DrawList_AddCircle(SPF_DrawList_Handle dl, float center_x, float center_y, float radius, uint32_t col, int num_segments, float thickness) {
  if (dl) ((ImDrawList*)dl)->AddCircle({center_x, center_y}, radius, col, num_segments, thickness);
}
void UIApi::UI_DrawList_AddCircleFilled(SPF_DrawList_Handle dl, float center_x, float center_y, float radius, uint32_t col, int num_segments) {
  if (dl) ((ImDrawList*)dl)->AddCircleFilled({center_x, center_y}, radius, col, num_segments);
}
...
void UIApi::UI_DrawList_AddCircleFilledMultiColor(SPF_DrawList_Handle dl, float center_x, float center_y, float radius, uint32_t col_inner, uint32_t col_outer, int num_segments) {
  ...
  for (int i = 0; i < num_segments; i++) {
    ...
    drawList->PrimWriteIdx(base_idx);
    drawList->PrimWriteIdx(base_idx + 1 + i);
    drawList->PrimWriteIdx(base_idx + 1 + j);
    drawList->PrimWriteIdx(base_idx + 1 + i);
    drawList->PrimWriteIdx(base_idx + 1 + j);
    drawList->PrimWriteIdx(base_idx + 1 + num_segments + j);
    drawList->PrimWriteIdx(base_idx + 1 + i);
    drawList->PrimWriteIdx(base_idx + 1 + num_segments + j);
    drawList->PrimWriteIdx(base_idx + 1 + num_segments + i);
  }
...
void UIApi::UI_DrawList_AddNgon(SPF_DrawList_Handle dl, float center_x, float center_y, float radius, uint32_t col, int num_segments, float thickness) {
  if (dl) ((ImDrawList*)dl)->AddNgon({center_x, center_y}, radius, col, num_segments, thickness);
}
void UIApi::UI_DrawList_AddNgonFilled(SPF_DrawList_Handle dl, float center_x, float center_y, float radius, uint32_t col, int num_segments) {
  if (dl) ((ImDrawList*)dl)->AddNgonFilled({center_x, center_y}, radius, col, num_segments);
}
...
void UIApi::UI_DrawList_AddBezierCubic(SPF_DrawList_Handle dl, float p1_x, float p1_y, float cp1_x, float cp1_y, float cp2_x, float cp2_y, float p2_x, float p2_y, uint32_t col, float thickness, int num_segments) {
  if (dl) ((ImDrawList*)dl)->AddBezierCubic({p1_x, p1_y}, {cp1_x, cp1_y}, {cp2_x, cp2_y}, {p2_x, p2_y}, col, thickness, num_segments);
}
void UIApi::UI_DrawList_AddBezierQuadratic(SPF_DrawList_Handle dl, float p1_x, float p1_y, float cp_x, float cp_y, float p2_x, float p2_y, uint32_t col, float thickness, int num_segments) {
  if (dl) ((ImDrawList*)dl)->AddBezierQuadratic({p1_x, p1_y}, {cp_x, cp_y}, {p2_x, p2_y}, col, thickness, num_segments);
}
...
void UIApi::UI_DrawList_AddConcavePolyFilled(SPF_DrawList_Handle dl, const float* points_x, const float* points_y, int num_points, uint32_t col) { UI_DrawList_AddConvexPolyFilled(dl, points_x, points_y, num_points, col); }

void UIApi::UI_DrawList_AddImage(SPF_DrawList_Handle dl, void* user_texture_id, float p_min_x, float p_min_y, float p_max_x, float p_max_y, float uv_min_x, float uv_min_y, float uv_max_x, float uv_max_y, uint32_t col) {
  if (dl) ((ImDrawList*)dl)->AddImage(user_texture_id, {p_min_x, p_min_y}, {p_max_x, p_max_y}, {uv_min_x, uv_min_y}, {uv_max_x, uv_max_y}, col);
}
void UIApi::UI_DrawList_AddImageQuad(SPF_DrawList_Handle dl, void* user_texture_id, float p1_x, float p1_y, float p2_x, float p2_y, float p3_x, float p3_y, float p4_x, float p4_y, float uv1_x, float uv1_y, float uv2_x, float uv2_y, float uv3_x,
                                     float uv3_y, float uv4_x, float uv4_y, uint32_t col) {
  if (dl) ((ImDrawList*)dl)->AddImageQuad(user_texture_id, {p1_x, p1_y}, {p2_x, p2_y}, {p3_x, p3_y}, {p4_x, p4_y}, {uv1_x, uv1_y}, {uv2_x, uv2_y}, {uv3_x, uv3_y}, {uv4_x, uv4_y}, col);
}
void UIApi::UI_DrawList_AddImageRounded(SPF_DrawList_Handle dl, void* user_texture_id, float p_min_x, float p_min_y, float p_max_x, float p_max_y, float uv_min_x, float uv_min_y, float uv_max_x, float uv_max_y, uint32_t col, float rounding,
                                        SPF_DrawFlags flags) {
  if (dl) ((ImDrawList*)dl)->AddImageRounded(user_texture_id, {p_min_x, p_min_y}, {p_max_x, p_max_y}, {uv_min_x, uv_min_y}, {uv_max_x, uv_max_y}, col, rounding, (ImDrawFlags)flags);
}
void UIApi::UI_DrawList_AddCallback(SPF_DrawList_Handle dl, void (*callback)(const void* parent_list, const void* cmd), void* user_data) {
  if (dl) ((ImDrawList*)dl)->AddCallback((ImDrawCallback)callback, user_data);
}

void UIApi::UI_DrawList_AddText(SPF_DrawList_Handle dl, float pos_x, float pos_y, uint32_t col, const char* text) {
  if (dl && text) ((ImDrawList*)dl)->AddText({pos_x, pos_y}, col, text);
}
...
void UIApi::UI_DrawList_PathClear(SPF_DrawList_Handle dl) {
  if (dl) ((ImDrawList*)dl)->PathClear();
}
void UIApi::UI_DrawList_PathLineTo(SPF_DrawList_Handle dl, float pos_x, float pos_y) {
  if (dl) ((ImDrawList*)dl)->PathLineTo({pos_x, pos_y});
}
void UIApi::UI_DrawList_PathStroke(SPF_DrawList_Handle dl, uint32_t col, SPF_DrawFlags flags, float thickness) {
  if (dl) ((ImDrawList*)dl)->PathStroke(col, (ImDrawFlags)flags, thickness);
}
void UIApi::UI_DrawList_PathFillConvex(SPF_DrawList_Handle dl, uint32_t col) {
  if (dl) ((ImDrawList*)dl)->PathFillConvex(col);
}
void UIApi::UI_DrawList_PathArcTo(SPF_DrawList_Handle dl, float center_x, float center_y, float radius, float a_min, float a_max, int num_segments) {
  if (dl) ((ImDrawList*)dl)->PathArcTo({center_x, center_y}, radius, a_min, a_max, num_segments);
}
void UIApi::UI_DrawList_PathRect(SPF_DrawList_Handle dl, float rect_min_x, float rect_min_y, float rect_max_x, float rect_max_y, float rounding, SPF_DrawFlags flags) {
  if (dl) ((ImDrawList*)dl)->PathRect({rect_min_x, rect_min_y}, {rect_max_x, rect_max_y}, rounding, (ImDrawFlags)flags);
}

void UIApi::UI_DrawList_ChannelsSplit(SPF_DrawList_Handle dl, int count) {
  if (dl) ((ImDrawList*)dl)->ChannelsSplit(count);
}
void UIApi::UI_DrawList_ChannelsMerge(SPF_DrawList_Handle dl) {
  if (dl) ((ImDrawList*)dl)->ChannelsMerge();
}
void UIApi::UI_DrawList_ChannelsSetCurrent(SPF_DrawList_Handle dl, int n) {
  if (dl) ((ImDrawList*)dl)->ChannelsSetCurrent(n);
}

void UIApi::UI_DrawList_PrimReserve(SPF_DrawList_Handle dl, int idx_count, int vtx_count) {
  if (dl) ((ImDrawList*)dl)->PrimReserve(idx_count, vtx_count);
}
void UIApi::UI_DrawList_PrimRectUV(SPF_DrawList_Handle dl, float p_min_x, float p_min_y, float p_max_x, float p_max_y, float uv_min_x, float uv_min_y, float uv_max_x, float uv_max_y, uint32_t col) {
  if (dl) ((ImDrawList*)dl)->PrimRectUV({p_min_x, p_min_y}, {p_max_x, p_max_y}, {uv_min_x, uv_min_y}, {uv_max_x, uv_max_y}, col);
}
void UIApi::UI_DrawList_PrimVtx(SPF_DrawList_Handle dl, float x, float y, float u, float v, uint32_t col) {
  if (dl) ((ImDrawList*)dl)->PrimVtx({x, y}, {u, v}, col);
}
void UIApi::UI_DrawList_PrimIdx(SPF_DrawList_Handle dl, uint16_t idx) {
  if (dl) ((ImDrawList*)dl)->PrimWriteIdx(idx);
}
...
uint32_t UIApi::UI_DockBuilderSplitNode(uint32_t node_id, SPF_Dir split_dir, float size_ratio, uint32_t* out_id_at_dir, uint32_t* out_id_at_opposite) {
  return ImGui::DockBuilderSplitNode(node_id, (ImGuiDir)split_dir, size_ratio, (ImGuiID*)out_id_at_dir, (ImGuiID*)out_id_at_opposite);
}
...
void UIApi::UI_SetLastItemData(uint32_t item_id, SPF_InputTextFlags flags, SPF_HoveredFlags status_flags, float min_x, float min_y, float max_x, float max_y) {
  ImGui::SetLastItemData(item_id, (ImGuiItemFlags)flags, (ImGuiItemStatusFlags)status_flags, {min_x, min_y, max_x, max_y});
}
bool UIApi::UI_ButtonBehavior(float min_x, float min_y, float max_x, float max_y, uint32_t id, bool* out_hovered, bool* out_held, SPF_WindowFlags flags) {
  return ImGui::ButtonBehavior({min_x, min_y, max_x, max_y}, id, out_hovered, out_held, (ImGuiButtonFlags)flags);
}
bool UIApi::UI_DragBehavior(uint32_t id, SPF_DataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, SPF_SliderFlags flags) {
  return ImGui::DragBehavior(id, (ImGuiDataType)data_type, p_v, v_speed, p_min, p_max, format, (ImGuiSliderFlags)flags);
}
...
void UIApi::UI_RenderTextClipped(float min_x, float min_y, float max_x, float max_y, const char* text, const char* text_end, float* out_text_size, float align_x, float align_y) {
  ImVec2 sz;
  ImGui::RenderTextClipped({min_x, min_y}, {max_x, max_y}, text, text_end, &sz, {align_x, align_y});
  if (out_text_size) {
    out_text_size[0] = sz.x;
    out_text_size[1] = sz.y;
  }
}
void UIApi::UI_RenderTextEllipsis(float x, float y, float max_width, const char* text) { ImGui::RenderTextEllipsis(ImGui::GetWindowDrawList(), ImVec2(x, y), ImVec2(x + max_width, y + 30.0f), x + max_width, text, nullptr, nullptr); }
...
void UIApi::UI_RenderBullet(SPF_DrawList_Handle dl, float x, float y, uint32_t col) {
  if (dl) ImGui::RenderBullet((ImDrawList*)dl, {x, y}, col);
}
...
void UIApi::UI_HideNotification(SPF_Notification_Handle handle) { UIManager::GetInstance().HideNotification(handle); }
void UIApi::UI_PlayTransition(SPF_TransitionType type, float duration, bool reverse, SPF_TransitionColor color) { UIManager::GetInstance().PlayTransition((int)type, duration, reverse, (int)color); }
bool UIApi::UI_IsTransitionActive() { return UIManager::GetInstance().IsTransitionActive(); }

void UIApi::UI_SetMouseBlockState(bool blockAxes, bool blockButtons, bool blockWheel) { Input::InputManager::GetInstance().SetProgrammaticMouseBlock(blockAxes, blockButtons, blockWheel); }
...
void* UIApi::UI_CreateTextureFromMemory(const void* data, size_t size, int* out_width, int* out_height) { return UIManager::GetInstance().CreatePluginTexture(data, size, out_width, out_height); }

void* UIApi::UI_CreateTextureFromFile(const char* file_path, int* out_width, int* out_height) { return UIManager::GetInstance().CreatePluginTextureFromFile(file_path, out_width, out_height); }

void UIApi::UI_DestroyTexture(void* texture_id) { UIManager::GetInstance().DestroyPluginTexture(texture_id); }
