...
void MarkdownRenderer::Render(const std::string& markdownText) {
  ...
  MD_PARSER parser = {0, MD_DIALECT_GITHUB | MD_FLAG_UNDERLINE | MD_FLAG_TABLES | MD_FLAG_TASKLISTS, OnEnterBlock, OnLeaveBlock, OnEnterSpan, OnLeaveSpan, OnText, nullptr, nullptr};
  ...
  switch (type) {
    ...
    case MD_BLOCK_QUOTE:
      if (enter) {
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(p.x + 4.0f, m_quoteStartY), ImVec2(p.x + 7.0f, endY), ImGui::GetColorU32(accentColor));
...
void MarkdownRenderer::HandleSpan(MD_SPANTYPE type, void* detail, bool enter) {
  switch (type) {
    case MD_SPAN_EM:
      m_style.isItalic = enter;
      break;
    case MD_SPAN_STRONG:
      m_style.isBold = enter;
      break;
    case MD_SPAN_U:
      m_style.isUnderline = enter;
      break;
    case MD_SPAN_DEL:
      m_style.isStrikethrough = enter;
      break;
    case MD_SPAN_CODE:
      m_style.isCode = enter;
      break;
    case MD_SPAN_A: {
...
void MarkdownRenderer::HandleText(MD_TEXTTYPE type, const char* text, MD_SIZE size) {
  ...
  if (m_isWaitingForAlertMarker && m_style.isBlockQuote) {
    ...
    size_t skipLen = 0;
    if (textStr.find("[!NOTE]") == 0) {
      m_style.alertType = AlertType::Note;
      skipLen = 7;
    } else if (textStr.find("[!TIP]") == 0) {
      m_style.alertType = AlertType::Tip;
      skipLen = 6;
    } else if (textStr.find("[!IMPORTANT]") == 0) {
      m_style.alertType = AlertType::Important;
      skipLen = 12;
    } else if (textStr.find("[!WARNING]") == 0) {
      m_style.alertType = AlertType::Warning;
      skipLen = 10;
    } else if (textStr.find("[!CAUTION]") == 0) {
      m_style.alertType = AlertType::Caution;
      skipLen = 10;
    }
    ...
    while (next_tag < end) {
      if (*next_tag == '<') {
        ...
        if (next_tag + 8 < end && *(next_tag + 1) == '#') {
          ...
          if (sscanf(next_tag + 2, "%02x%02x%02x", &tr, &tg, &tb) == 3 && next_tag[8] == '>') break;
        }
      }
      next_tag++;
    }
