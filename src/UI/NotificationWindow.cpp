...
SPF_Notification_Handle NotificationWindow::ShowEx(const SPF_Notification_Params& params) {
  ...
  if (params.mode == SPF_NOTIF_MODE_STICKY) {
    auto it = std::find_if(m_notifications.begin(), m_notifications.end(), [&](const NotificationData& n) { return n.mode == SPF_NOTIF_MODE_STICKY && n.message == processedMessage; });
...
void NotificationWindow::Hide(SPF_Notification_Handle handle) {
  ...
  auto it = std::find_if(m_notifications.begin(), m_notifications.end(), [&](const NotificationData& n) { return n.handle == h; });
  if (it != m_notifications.end()) {
    it->isClosing = true;
  }
}
...
void NotificationWindow::RenderContent() {
  ...
  if (clickedOutside && !anyStickyHovered) {
    m_notifications.erase(std::remove_if(m_notifications.begin(), m_notifications.end(), [&](const NotificationData& n) { return n.mode == SPF_NOTIF_MODE_STICKY && !n.isProgrammatic && n.initialized; }), m_notifications.end());
  }
}
...
void NotificationWindow::RenderSingleNotification(NotificationData& notif, int index) {
  ...
  ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing |
  ...
  if (ImGui::Begin(windowName.c_str(), nullptr, flags)) {
    ...
    if (notif.mode != SPF_NOTIF_MODE_STICKY) {
      ...
      drawList->AddRectFilled(p_min, ImVec2(p_min.x + avail_w, p_min.y + bar_h), ImGui::GetColorU32(ImGuiCol_FrameBg), 1.0f);

      // Draw progress bar if visible
      if (progress > 0.0f) {
        drawList->AddRectFilled(p_min, ImVec2(p_min.x + (avail_w * progress), p_min.y + bar_h), ImGui::ColorConvertFloat4ToU32(color), 1.0f);
      }
...
ImGuiWindowFlags NotificationWindow::GetExtraWindowFlags() const {
  return ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoDecoration |
