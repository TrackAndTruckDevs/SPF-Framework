...
WelcomeWindow::WelcomeWindow(const std::string& componentName, const std::string& windowId) : m_componentName(componentName), m_windowId(windowId) {
  m_isVisible = false;
  UpdateLocalization();
}
...
void WelcomeWindow::RenderMarkdownContent() {
  ...
  if (m_mode == WelcomeMode::FirstInstall) {
    ...
    Typography::Text(TextStyle::H2().Separator().Color(Colors::GOLD).Align(TextAlign::Center).Wrapped(), "%s", m_locIntroTitle.c_str());
    ImGui::PopStyleVar();
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_SeparatorTextAlign, ImVec2(0.5f, 0.5f));
    Typography::Text(TextStyle::H2().Separator().Color(Colors::GOLD).Align(TextAlign::Center).Wrapped(), "%s", m_updateTitle.c_str());
    ImGui::PopStyleVar();
