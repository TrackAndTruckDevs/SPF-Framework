    ...
    if (!m_editingBindingDetails.has_value() || !m_editingBindingAction.has_value()) {
      ...
      if (selectedPressTypeStr != currentPressTypeStr) {
        ...
        if (input && input->IsValid()) {
          ...
          Input::PressType newPressType =
              (selectedPressTypeStr == "long") ? Input::PressType::Long : Input::PressType::Short;  // Simplified for now
      ...
      if (m_pressTypeSwapConflict.has_value()) {
        ...
        if (!m_pressTypeSwapNewValue.has_value() || !m_editingBindingAction.has_value()) {
          ...
          ImGui::Text("%s", loc.GetFormatted(m_currentComponent, m_conflictPressTypeMessage, loc.Get("enums.press_type." + newPressType)).c_str());
          ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "%s", getTranslatedActionName(conflictingActionName).c_str());
          ImGui::Text("%s", loc.Get(m_conflictSwapQuestion).c_str());
          
          if (ImGui::Button(loc.Get(m_conflictYesSwapButton).c_str())) {
          ...
          ImGui::SameLine();
          if (ImGui::Button(loc.Get(m_conflictCancelButton).c_str())) {
