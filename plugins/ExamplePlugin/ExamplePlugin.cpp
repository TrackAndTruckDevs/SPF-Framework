...
void BuildManifest(SPF_Manifest_Builder_Handle* h, const SPF_Manifest_Builder_API* api) {
  ...
  // This section provides the basic identity of your plugin.
  {
    ...
    api->Info_SetMinFrameworkVersion(h, "1.2.1");
...
void OnUpdate() {
  ...
  auto strcat_safe = [&](const char* src) { strcat_s(full_log_buffer, sizeof(full_log_buffer), src); };
  ...
  // --- Navigation ---
  {
    ...
    format->Fmt_Format(
      temp_line_buffer, sizeof(temp_line_buffer), "  Distance: %.0f m, Time: %.0f s (%.1f real s), Speed Limit: %.0f kph\n", data.navigation_distance, data.navigation_time, data.navigation_time_real_seconds, data.navigation_speed_limit * 3.6f);
    strcat_safe(temp_line_buffer);
  }
  ...
  // --- Controls ---
  {
    ...
    format->Fmt_Format(temp_line_buffer, sizeof(temp_line_buffer), "  User: Thr=%.2f, Brk=%.2f, Steer=%.2f, Clutch=%.2f\n", data.userInput.throttle, data.userInput.brake, data.userInput.steering, data.userInput.clutch);
    strcat_safe(temp_line_buffer);
    format->Fmt_Format(temp_line_buffer, sizeof(temp_line_buffer), "  Effective: Thr=%.2f, Brk=%.2f, Steer=%.2f, Clutch=%.2f\n", data.effectiveInput.throttle, data.effectiveInput.brake, data.effectiveInput.steering, data.effectiveInput.clutch);
    strcat_safe(temp_line_buffer);
  }
...
/**
 * @brief Renders the content for the "Game World" tab.
 */
void RenderGameWorldTab(SPF_UI_API* ui, void* user_data) {
  ...
  // To prevent skyboxAutoUpdate from resetting every frame, we use skyboxLock status
  // Note: skyboxAutoUpdate is set to true by default, meaning skyboxLock is false.
  static bool skyboxLock = false;

  // We check the checkbox. If it changes, update the engine.
  ...
  // Disable slider if visual time is not locked
  if (!skyboxLock) {
...
void InitializeVirtualDevice(SPF_VirtInput_API* input_api, SPF_Logger_API* logger_api) {
  ...
  g_ctx.virtualDevice = input_api->Virt_CreateDevice(PLUGIN_NAME, "Example_virtual_device", "ExamplePlugin Virtual Controller", SPF_INPUT_DEVICE_TYPE_GENERIC);
